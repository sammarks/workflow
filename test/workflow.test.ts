...
describe('#execute()', () => {
  ...
  beforeEach(() => {
    ...
    firstStep = jest.fn(async (context) => {
      expect(context).toStrictEqual({ another: 'bar' })
    })
    ...
    thirdStep = jest.fn(async (context) => {
      expect(context).toStrictEqual({ another: 'bar', second: true })
    })
  ...
  describe('when all steps succeed', () => {
    ...
    beforeEach(async () => {
      result = await execute<TestContext>('test-workflow', [
    ...
    it('calls each step with the proper context', () => {
      expect(firstStep.mock.calls).toStrictEqual([[result]])
      expect(firstStepFallback.mock.calls).toStrictEqual([])
      expect(secondStep.mock.calls).toStrictEqual([[result]])
      expect(thirdStep.mock.calls).toStrictEqual([[result]])
      expect(thirdStepFallback.mock.calls).toStrictEqual([])
    })
  ...
  describe('when one step fails', () => {
    beforeEach(async () => {
      try {
        await execute<TestContext>('test-workflow', [
          { name: 'first', run: firstStep, revert: firstStepFallback },
          { name: 'second', run: secondStep },
    ...
    it('calls the revert steps', () => {
      expect(failStepRevertNoError.mock.calls).toStrictEqual([[{ another: 'bar', second: true, fallback: true }]])
      expect(firstStepFallback.mock.calls).toStrictEqual([[{ another: 'bar', second: true, fallback: true }]])
    })
  })
